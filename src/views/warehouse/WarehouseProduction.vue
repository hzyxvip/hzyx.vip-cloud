...
import { findProductInList } from '@/utils/productStore'
import {
  loadBatchNoFormat,
  saveBatchNoFormat,
...
const createEmptyItem = (): ProductionItem => ({
  id: Date.now(),
  productCode: '',
  productName: '',
  spec: '',
...
const items = ref<ProductionItem[]>([createEmptyItem()])
...
const resolveItemProduct = (row: ProductionItem) =>
  row.productCode ? findProductInList(row.productCode.trim()) : undefined

const handleBatchNoFormatChange = (format: BatchNoFormat) => {
  saveBatchNoFormat(format)
  reapplyBatchNoFormatToItems(items.value as Record<string, any>[], format)
}

const handleProductionDateChange = (row: ProductionItem) => {
  const product = resolveItemProduct(row)
  applyProductionDateToItemRow(row as Record<string, any>, product, batchNoFormat.value)
}

const handleBatchNoInput = (row: ProductionItem) => {
  markBatchNoManual(row as Record<string, any>, batchNoFormat.value)
}

const handleExpiryDateChange = (row: ProductionItem) => {
  markExpiryManual(row as Record<string, any>)
}

const handleProductCodeBlur = (row: ProductionItem) => {
  ...
  row.manufacturer = product.manufacturer || row.manufacturer
  row.unit = product.measureUnit || product.purchaseUnit || row.unit
          ...
          <el-table-column label="商品编码" width="140">
            <template #default="scope">
              <el-input
                ...
                placeholder="商品编码"
                @blur="handleProductCodeBlur(scope.row)"
              />
            </template>
          </el-table-column>
          ...
          <el-table-column label="批号" width="150">
            <template #default="scope">
              <BatchNoCell
                ...
                @format-change="handleProductionDateChange(scope.row)"
                @batch-input="handleBatchNoInput(scope.row)"
              />
            </template>
          </el-table-column>
          ...
          <el-table-column label="有效期至" width="130">
            <template #default="scope">
              <el-date-picker
                ...
                style="width: 100%;"
                clearable
                @change="handleExpiryDateChange(scope.row)"
              />
            </template>
          </el-table-column>
