...
const payableRows = ref(buildPayableDetailRows())

const paymentOptions = computed(() =>
  listFundDocuments('payment')
...
const partnerOptions = computed(() => {
  ...
  listFundDocuments('payment').forEach(d => {
...
const filteredPayables = computed(() =>
  payableRows.value.filter(row => !partner.value || row.partner === partner.value)
)

onMounted(() => {
  payableRows.value = buildPayableDetailRows()
...
<template>
  <div class="page-container">
    ...
    <div class="form-card">
        ...
        <el-row :gutter="16">
          <el-col :xs="24" :sm="12">
            <el-form-item label="供应商">
              <el-select v-model="partner" filterable clearable placeholder="选择供应商">
          ...
          <el-col :xs="24" :sm="12">
            <el-form-item label="付款单">
              <el-select
                ...
                filterable
                placeholder="选择待核销付款单"
                @change="handleDocChange"
              >
                <el-option v-for="opt in paymentOptions" :key="opt.value" v-bind="opt" />
              </el-select>
            </el-form-item>
          </el-col>
        </el-row>
      ...
      <h3 class="section-title">待付明细（参考）</h3>
      <div class="table-scroll">
        <el-table :data="filteredPayables" border size="small">
