...
import { buildPayableAgeRows } from '@/utils/fundReportService'
import { formatFundAmount } from '@/utils/fundStore'
import { buildFundExportFileName, exportRowsToExcel } from '@/utils/fundExcelExport'

const tableData = ref(buildPayableAgeRows())

onMounted(() => {
  document.title = '应付账款账龄表'
  refreshData()
})

const refreshData = () => {
  tableData.value = buildPayableAgeRows()
}
...
<template>
  <div class="page-container">
    ...
    <div class="table-card">
      ...
      <el-empty v-if="!tableData.length" description="暂无未付应付数据" />
    </div>
  </div>
</template>
