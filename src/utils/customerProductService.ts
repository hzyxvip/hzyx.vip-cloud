...
const mapSalesLineToLocal = (line: Record<string, unknown>): ProductMaster => ({
  ...
  licenseNo: String(line.productionLicenseNo || ''),
  ...
  items.forEach(line => {
    ...
    if (idx >= 0) {
      list[idx] = {
        ...
        licenseNo: String(line.productionLicenseNo || list[idx].licenseNo),
        storageCondition: String(line.storageCondition || list[idx].storageCondition),
        bidType: String(line.bidType || list[idx].bidType || ''),
